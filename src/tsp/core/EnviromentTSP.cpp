#include "EnviromentTSP.hpp"

EnviromentTSP::EnviromentTSP(const std::vector<City>& cities, const std::vector<Connection>& connetions) : n (cities.size()){
    n = cities.size();
...
EnviromentTSP::~EnviromentTSP(){}

const std::vector<std::vector<double> > &EnviromentTSP::getMatrix() const{
    return matrix;
}

double EnviromentTSP::getNormalizer() const{
    return normalizer;
}

double EnviromentTSP::getMaxDS() const{
    return maxDS;
}